define(require => {
  ...
  let numberOfWinningNumbers = 3;
  let numberOfPlayerNumbers = 16;
  ...
  function start() {
    ...
    if (currentGameState === 'BASE_GAME') {
      ...
      revealAllTimeline.add(
        new Timeline({
          tweens: revealWinning,
          stagger: gameConfig.autoPlayWinningNumberInterval
        })
      );
      ...
      revealAllTimeline.add(
        new Timeline({
          tweens: revealPlayer,
          stagger: gameConfig.autoPlayPlayerNumberInterval
        }),
    ...
    if (currentGameState === "CollectBonus") {
      ...
      revealAllTimeline.add(
        new Timeline({
          tweens: collectBonusTweens,
          stagger: gameConfig.autoPlayCollectBonusReveal
        })
      );
      return revealAllTimeline;
    }
