define(require => {
  ...
  function enable() {
    ...
    return cards.map(async card => {
      ...
      // We have STARTED to reveal this number
      msgBus.publish('Game.WinningNumberStart', nextNumber);
  ...
  function checkMatch(playerNumber) {
    ...
    // Check to see if this card has been 100% revealed
    if (matchedCard && matchedCard.uncovered) {
      if (!autoPlay.enabled) {
        matchedCard.match();
