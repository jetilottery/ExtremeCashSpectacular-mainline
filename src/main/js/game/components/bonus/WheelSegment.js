    ...
    class Segment extends PIXI.Container {
        constructor(data) {
            ...
            if (this.type === 'outer') {
                ...
                this.texts[0].anchor.set(0.8, 0.6);
                this.texts[1].anchor.set(0.8, 0.6);
                this.texts[0].align = 'right';
                this.texts[1].align = 'right';
        ...
        update() {
            if (typeof this.data === 'string') {
                if (this.type === 'inner') {
                    ...
                    this.texts[0].text = SKBeInstant.formatCurrency(this.prize).formattedAmount.replace(" ", "\u0020");
                    this.texts[1].text = SKBeInstant.formatCurrency(this.prize).formattedAmount.replace(" ", "\u0020");
                } else {
        ...
        land() {
            ...
            function text0Out(winText) {
                tl.to(winText.scale, scaleTime, {
                    x: scaleAmount,
                    y: scaleAmount,
                    ease: window.Power2.easeOut
                }, 0);
                tl.to(winText, scaleTime, {
                    alpha: 0,
                    ease: window.Power2.easeOut
                }, 0.02);
            }
