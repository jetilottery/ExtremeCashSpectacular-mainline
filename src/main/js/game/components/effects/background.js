...
define(function(require) {
  ...
  var displayList = require('skbJet/componentManchester/standardIW/displayList'); // const SKBeInstant = require('skbJet/component/SKBeInstant/SKBeInstant');
  // const meterData = require('skbJet/componentManchester/standardIW/meterData');


  var resLib = require('skbJet/component/resourceLoader/resourceLib');

  var orientation = require('skbJet/componentManchester/standardIW/orientation'); // const gameConfig = require('skbJet/componentManchester/standardIW/gameConfig');
  ...
  var prizeBonusElements = ['BGambient_BONUS', 'PORTRAIT/PORT_BGambient_BONUS'];
  var wheelBonusElements = ['BGambient_BONUS_Wheel'];
  var baseGameElementsAnim = ['BGambient', 'PORTRAIT/PORT_BGambient']; // const bgShapes = ['BGSHAPES','PORT_BGSHAPES'];
  // const bgShapesBonus = ['BGSHAPES_BONUS','PORT_BGSHAPES'];

  var basegameBackgroundLayer;
  var bonusBackgroundLayer;
  var elementsLayer;
  var shapesLayer;
  var spineAnim_Base;
  var spineAnim_Elements;
  var spineAnim_Shapes;
  var spineAnim_Shapes_Bonus;
  var spineAnim_Bonus;
  var spineAnim_BonusPort; // let pricePointIndex;
  ...
  var _bonusName; // Map config settings to spine animation names
  // const colourTable = {
  //     'Red': 'RED',
  //     'Grey': 'GREY',
  //     'Pink': 'PINK',
  ...
  function init() {
    ...
    shapesLayer = displayList.backgroundShapes; // Set up spine project
    ...
    spineAnim_BonusPort.renderable = false; // Add to background containers
    ...
    spineAnim_Bonus.state.setAnimation(0, 'BLUE', false); // spineAnim_BonusPort.state.setAnimation(0, 'PORTRAIT/PORT_BLUE', false);
    ...
    msgBus.subscribe('PrizeData.PrizeStructure', switchBackground); // msgBus.subscribe('game.background.update',updateBonusBackround);
  ...
  function switchBackground() {
    if (spineAnim_Base) { //     let prices = SKBeInstant.config.gameConfigurationDetails.availablePrices;
  ...
  function transitionBackground(data) {
    return new Promise(function(resolve) {
      ...
      _bonusName = data.bonusName;
      console.log("DATA: " + data);
      console.log("TO BONUS: " + _toBonus);
      console.log("BONUS NAME: " + _bonusName);
      ...
      if (_toBonus) {
        ...
        spineAnim_Bonus.alpha = 1; // fade out the current bg elements
        ...
        Tween.to([spineAnim_Base, spineAnim_Elements, spineAnim_Shapes], data.delay, {
          ...
          onComplete: function onComplete() {
            ...
            updateBonusBackround(); // once the elements has faded out and switched, fade it back in, showing the new one
        ...
        var targetIndex = displayList.background.getChildIndex(bonusBackgroundLayer);
        displayList.background.setChildIndex(basegameBackgroundLayer, targetIndex); // make basegame bg visible and alpha 1

        spineAnim_Base.renderable = true;
        spineAnim_Base.alpha = 1;
        basegameBackgroundLayer.visible = true;
        basegameBackgroundLayer.alpha = 1; // fade the bonus background and BG elements out
        ...
        Tween.to([spineAnim_Bonus, spineAnim_BonusPort, spineAnim_Elements, spineAnim_Shapes_Bonus], data.delay, {
          ...
          onComplete: function onComplete() {
            ...
            spineAnim_Shapes_Bonus.renderable = false;
            spineAnim_Elements.renderable = true;
            spineAnim_Elements.state.setAnimation(0, baseGameElementsAnim[orientation.get() === orientation.LANDSCAPE ? 0 : 1], true);
          }
        });
  ...
  function updateBonusBackround() {
    ...
    if (_toBonus) {
      console.log("UPDATING BONUS ASSETS: " + _bonusName);

      if (_bonusName === 'CollectBonus') {
        spineAnim_Elements.renderable = true;
        spineAnim_Elements.state.setAnimation(0, prizeBonusElements[orientation.get() === orientation.LANDSCAPE ? 0 : 1], true);
      } else {
        if (orientation.get() === orientation.LANDSCAPE) {
          spineAnim_Elements.renderable = true;
          spineAnim_Elements.state.setAnimation(0, wheelBonusElements[0], true);
        } else {
          spineAnim_Elements.renderable = false;
        }
      }
    } else {
  ...
  function updateBackgroundElements(name) {
    this.name = name;
    console.log("UPDATING... " + this.name);

    _bonusName = this.name;
